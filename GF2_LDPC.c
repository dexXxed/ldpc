...
unsigned int Rand(void)           // псевдослучайное число
{
  return rndm = (77UL * rndm + 1243UL) & 0x7fffffffUL; // 31 бит
}
double atanh2(double x)
{
  return log((1.0 + x) / (1.0 - x));  // возвращает 2*atanh(x)
}
double logtanh2(double x)
{
  return log(tanh(fabs(x*0.5)));  // возвращает log tanh |x|
}

#define INT	6/*8*/              // целая часть
#define DECI	14/*13*/              // дробная часть
#define FMUL	(1<<DECI)       // множитель
#define PREC	(1.0/FMUL)      // точность исчислений
...
int **malloc2Dint(int a, int b) // выделяет массив[a][b]
{
  int i;
...
int dec(int q0[], int s[], int loop_max, int x[])
{
  ...
  for (loop = 0; loop < loop_max; loop++) {
    ...
    if (i == 0)           // ничего больше не может быть выполнено
      return 0;
    ...
    if (i > iir * 1.1 || nodecr > 10) break; // без преобразования
    prev = i;
  }
...
void initdec(char *s)
{
  ...
  for (j = 0; j < m; j++) {
    for (i = 0; i < row_weight[j]; i++) {
      ...
      fscanf(fp, "%d", &v);
      v--;
      row_col[j][i] = v;	// адрес колонки
      row_N[j][i] = count[v];	// вертикальный счетчик ненулевого коэффициента
      col_row[v][count[v]] = j;	// адрес строчк
      col_N[v][count[v]] = i;	// горизонтальный счетчик ненулевого коэффициента
      count[v]++;
...
int main(int argc, char **argv)
{
  ...
  s = malloc(sizeof(int) * m);  // синдром
  x = malloc(sizeof(int) * n);  // источник
  y = malloc(sizeof(int) * n);  // дополнительная информация
  for (i = 0; i < n; i++) {
    x[i] = Rand() & 1;
  }
  enc(x, s);
  ...
  for (i = 1; i <= 3; i++) {
    ...
    puts(
      dec(q0, s, iteration, x) ? // декодирование суммы произведения (3-й аргумент: максимальная итерация)
      "failed." : "converged."
      );
  }
